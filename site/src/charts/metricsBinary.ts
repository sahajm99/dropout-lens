...
/** Accuracy is the default picker choice, per the brief's title example. */
...
function findResult(
  ...
  return data.results.find(
    (r) => r.target === "binary" && r.feature_set === fs && r.model === model,
  );
}
...
function specFor(data: ModelMetrics, metric: MetricDef): FigureSpec {
  const bestEnrolModel = data.best.binary.enrolment;
  const bestAfterModel = data.best.binary.after_s1;
  ...
  const title =
    vEnrol && vAfter
      ? `At enrolment the best model reaches ${metric.fmt(vEnrol.value)} ${metric.label} (${lowerLead(data.model_labels[bestEnrolModel])}); after the first semester, ${metric.fmt(vAfter.value)}`
      : `At enrolment the best model is ${lowerLead(data.model_labels[bestEnrolModel])}; after the first semester it is ${lowerLead(data.model_labels[bestAfterModel])}`;
  ...
  return {
    ...
    note: `Train ${int(data.split.n_train)}, test ${int(data.split.n_test)}, split stratified on ${data.split.stratified_on}; the majority baseline always predicts "${data.targets.binary.classes[0]}".${undefinedForBaseline ? " ROC AUC and PR AUC are not defined for that baseline, so it is left off this view." : ""}`,
    alt: `Dot plot of four models' ${metric.label} at enrolment and after the first semester, with bootstrap intervals. ${title}.`,
    table: {
...
function metricPicker(
  ...
  select.id = "fig-metrics-binary-picker";
...
export async function render(container: HTMLElement): Promise<void> {
  ...
  let metric: MetricDef = METRICS[0]!;
